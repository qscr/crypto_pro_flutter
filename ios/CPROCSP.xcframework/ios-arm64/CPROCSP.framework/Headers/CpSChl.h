...
//для сервера означают, что получен от клиента и будет поддерживаться
    ...
    // Флаг client_renegotiation_info_sent нужен для запоминания того, 
    // что клиент в первом ClientHello посылал либо расширение renegotiation_info, либо сигнальную сюиту SCSV.
    // Если он в самом начале выставится в 1, то во всех пересогласованиях клиент должен посылать renegotiation_info
    // в независимости от того, поддерживает ли текущее соединение Secure Renegotiation или нет,
    // см. https://www.rfc-editor.org/rfc/rfc5746#section-3.5 и https://www.rfc-editor.org/rfc/rfc5746#section-4.2
    unsigned int client_renegotiation_info_sent : 1;
    unsigned int EnablePeerGostCurvesByCiphersServer : 1;
    unsigned int UseSessionTickets : 1;
...
#define LSA_CTX_ID_MAGIC ((DWORD)0x15AC781D)

typedef struct _SECURIY_HANDLE_{    
    // Признак структуры SECURITY_HANDLE
    DWORD					Magic;
    //наш контекст
...
#ifndef SECBUFFER_CERTIFICATE_REQUEST_CONTEXT
#define SECBUFFER_CERTIFICATE_REQUEST_CONTEXT 0x1D
...
#endif // SECBUFFER_CERTIFICATE_REQUEST_CONTEXT
...
// CPCSP-13930
#define MAX_ATTRS_NEGOTIATED_TLS_EXTENSIONS 7

#ifndef SECPKG_ATTR_NEGOTIATED_TLS_EXTENSIONS
