...
#include <CPROPKI/ocsp.h>
#include <CPROPKI/cplib/Blob.h>
	...
	#define OCSPCLI_CLASS __declspec(dllimport)
        ...
        #include <CPROPKI/pki_ocsp_client_assembly.h>
            #ifdef _WIN64
...
typedef enum {
    OCSPCLI_SKIP_IE_PROXY_CONFIGURATION = 1 << 0,
    // Возвращать ответ из кэша в общем случае нельзя (CPCSP-13257). Флаг означает "тут можно".
    OCSPCLI_CAN_READ_FROM_CACHE = 1 << 1,
    // Читать из сети в общем случае можно (CPCSP-13386). Флаг означает "тут нельзя".
    OCSPCLI_CANNOT_READ_FROM_WIRE = 1 << 2
} OCSPCLI_FLAGS;
...
namespace Client {
	...
	void put_CacheResync(FILETIME *pftCacheResync);
	void put_ChainEngine(HCERTCHAINENGINE hEngine);
